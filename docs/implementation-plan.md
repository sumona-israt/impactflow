...
Scenarios 2 (duplicate detection on registration) and 4 (bulk import) are functional as of Phase 5. Scenario 3 is functional up to "Finance approves" (Phase 4) — the Odoo sync step is still Phase 7. Scenario 1's "dashboard updates" clause is functional as of Phase 6 — the executive dashboard reflects real program/beneficiary/finance state, permission-gated per viewer. Scenario 5 remains Phase-7-only.
